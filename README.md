...
Self-тест рендер-ядра: <http://localhost:5173/selftest.html> (30 проверок — координаты
отрисовки, кольца, стиль detection, сетка и её эффект, затухание feedback, блендинг,
детекторы Хафа на синтетическом кадре, сериализация патча, ленивый импорт MediaPipe).
Страница только для дева: в `dist` она не собирается.
