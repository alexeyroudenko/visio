...
export const featuresGridNode = defineNode<GridState>({
  ...
  label: "Features Grid",
  category: "draw",
  description: "Мондриан-сетка: кадр рекурсивно режется по точкам трекинга, с подписями ячеек.",
  inputs: [
  ...
  params: [
    { key: "color", label: "Цвет", type: "color", default: "#f5f0e6" },
    { key: "maxDepth", label: "Глубина", type: "range", min: 1, max: 8, step: 1, default: 5 },
    { key: "minSize", label: "Мин. ячейка", type: "range", min: 16, max: 300, step: 4, default: 64 },
    { key: "stroke", label: "Толщина", type: "range", min: 0.5, max: 8, step: 0.5, default: 1 },
    { key: "opacity", label: "Прозрачность", type: "range", min: 0, max: 1, step: 0.05, default: 1 },
    { key: "labels", label: "Подписи", type: "toggle", default: true },
    { key: "labelSize", label: "Размер шрифта", type: "range", min: 8, max: 40, step: 1, default: 13 },
    { key: "labelText", label: "Текст", type: "text", default: "Element" },
    { key: "effectChance", label: "Доля ячеек с эффектом", type: "range", min: 0, max: 1, step: 0.05, default: 0 },
    { key: "effectSeed", label: "Seed эффекта", type: "range", min: 1, max: 999, step: 1, default: 42 },
  ],
  createState() {
